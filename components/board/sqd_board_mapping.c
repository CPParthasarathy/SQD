...
_Static_assert((sizeof(s_board_pins) / sizeof(s_board_pins[0])) == SQD_BOARD_SIGNAL_COUNT,
...
bool sqd_board_internal_mapping_is_valid(void) {
    ...
    for (first_index = 0; first_index < SQD_BOARD_SIGNAL_COUNT; ++first_index) {
        ...
        if ((first->gpio < GPIO_NUM_0) || (first->gpio >= GPIO_NUM_MAX)) {
            return false;
        }

        for (second_index = first_index + 1; second_index < SQD_BOARD_SIGNAL_COUNT; ++second_index) {
            const sqd_board_pin_t *second = &s_board_pins[second_index];
